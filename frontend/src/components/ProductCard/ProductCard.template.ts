...
<a href="/product/{{id}}" data-link class="product-card{{#if hidden}} product-card--hidden{{/if}}">
  ...
  <div class="product-card__body">
    ...
    <div class="product-card__price-row">
      ...
      {{#if (gt sale 0)}}<span class="product-card__price-old">{{formatPrice price}}</span>{{/if}}
    </div>
  </div>
</a>
`;
